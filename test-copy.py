...
def test(model,model_path,datapath,name_index):
    model = model.cuda()
    if model_path is not None:
        model.load_state_dict(torch.load(os.path.join(model_path, 'net_epoch23.pth')))
        print('load_model from ' + os.path.join(model_path, 'net_latest.pth'))
    model.eval()
    psnr_test ,pixel_metric,count,psnr_max,ssim_max = 0,0,0,0,0
    dataset = datapath
        ...
        # for curDir, dirs, files in os.walk("/data/ProjectData/Derain/Rain200L/test/rain"):
                for file in files:
                    ...
                    if loss >= ssim_max:
                        ssim_max = loss
                    count += out.shape[0]
                    Note=open('/data/ProjectData/Derain/xiaorong/mixDTPNet/test.txt','a')
                    ...
                    if 1: # 输出图像
                        ...
                        x = np.uint8(255 * x.cpu().numpy().squeeze())
                        r, g, b = cv2.split(x.transpose(1, 2, 0))
                        cv2.imwrite(f'/data/ProjectData/Derain/xiaorong/mixDTPNet/{count}.jpg',cv2.merge([b ,g, r]))
                    # except:
                    #     pass
                    # break
                    times += (endtime - begin)
...
if __name__ == "__main__":
    ...
    res += test(model,"/data/ProjectData/Derain/Rain200L/TrainedModel/mixDTPNet/Logs/200L","/data/ProjectData/Derain/Rain100L/rainy",count)
    print(res)    
