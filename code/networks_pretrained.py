...
class CTPnet(nn.Module):
    def __init__(self, recurrent_iter=3, use_GPU=True, factor=10e-3, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
        ...
        self.conv2_1_HSI = nn.Sequential(*[TransformerBlock(dim=int(dim*2**1), num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])]) # 256+128, out_channel, channel_att=channel_att, spatial_att=TRUE
        # 6~8层要先上采样再卷积 # 基于通道注意力的上采样
        self.conv4_3_HSV = nn.Sequential(*[TransformerBlock(dim=int(dim*2**2), num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])]) # 512+512, 512, channel_att=TRUE, spatial_att=spatial_att
        self.conv3_2_HSV = nn.Sequential(*[TransformerBlock(dim=int(dim*2**1), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])]) # 256+512, 256, channel_att=TRUE, spatial_att=spatial_att
        self.conv2_1_HSV = nn.Sequential(*[TransformerBlock(dim=int(dim*2**1), num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])]) # 256+128, out_channel, channel_att=TRUE, spatial_att=spatial_att
        # 6~8层要先上采样再卷积 # 直接上采样
        self.conv4_3_RGB = nn.Sequential(*[TransformerBlock(dim=int(dim*2**2), num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])]) # 512+512, 512,  channel_att=False, spatial_att=False
        self.conv3_2_RGB = nn.Sequential(*[TransformerBlock(dim=int(dim*2**1), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])]) # 256+512, 256, channel_att=False, spatial_att=False
        self.conv2_1_RGB = nn.Sequential(*[TransformerBlock(dim=int(dim*2**1), num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])]) # 256+128, out_channel, channel_att=False, spatial_att=False
        ...
        self.outc = nn.Sequential(*[TransformerBlock(dim=int(dim*2**1), num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_refinement_blocks)])
        self.conv_final = nn.Conv2d(int(dim*2**1), out_channel_color, kernel_size=3, stride=1, padding=1, bias=bias)
    ...
    def forward(self, input):
        ...
        # Method 2 预训练模型
        conv1_2,conv2_3,conv3_4 = self.vgg16(input)
        ...
        # input_level2 = self.down1_2(conv1_2) # torch.Size([18, 64, 96, 96]) -> torch.Size([18, 128, 48, 48]) 
        ...
        # 开始上采样  同时要进行 skip connection 不要一起进行，会爆显存
        for i in range(1):
            if i==0: # HSI采样
                # First Up
                inp_dec_level3 = self.up4_3(conv4_L) # torch.Size([18, 512, 12, 12]) -> torch.Size([18, 256, 24, 24])
                inp_dec_level3 = torch.cat([inp_dec_level3, conv3_4], 1) # torch.Size([18, 256, 24, 24]) -> torch.Size([18, 512, 24, 24])
                inp_dec_level3 = self.reduce_chan_level3(inp_dec_level3) # torch.Size([18, 512, 24, 24]) -> torch.Size([18, 256, 24, 24])
                conv4_3 = self.conv4_3_HSI(inp_dec_level3) # ([18, 256, 24, 24]) -> ([18, 256, 24, 24])
                # Second Up  
                inp_dec_level2 = self.up3_2(conv4_3) # torch.Size([18, 256, 24, 24]) -> 
                ...
                core1 = self.outc(conv2_1) # conv2_1.torch.Size([6, 128, 96, 96]) ->  core1.torch.Size([6, 128, 96, 96])
                pred1 = self.conv_final(core1) # pred1.torch.Size([6, 12, 96, 96])
            elif i==1:  # HSV采样
                # First Up
                inp_dec_level3 = self.up4_3(conv4_L)
                inp_dec_level3 = torch.cat([inp_dec_level3, conv3_4], 1)
                inp_dec_level3 = self.reduce_chan_level3(inp_dec_level3)
                conv4_3 = self.conv4_3_HSV(inp_dec_level3) # torch.cat([conv4, F.interpolate(conv4, scale_factor=2, mode=self.upMode)], dim=1)  self.upMode为 bilinear ([18, 512, 12, 12])
                # Second Up  
                inp_dec_level2 = self.up3_2(conv4_3)
                inp_dec_level2 = torch.cat([inp_dec_level2, conv2_3], 1)
                inp_dec_level2 = self.reduce_chan_level2(inp_dec_level2)
                conv3_2 = self.conv3_2_HSV(inp_dec_level2)
                # Third Up
                inp_dec_level1 = self.up2_1(conv3_2)
                inp_dec_level1 = torch.cat([inp_dec_level1, conv1_2], 1)
                conv2_1 = self.conv2_1_HSV(inp_dec_level1)
                # return channel K*K*N
                core2 = self.outc(conv2_1)
                ...
                inp_dec_level3 = torch.cat([inp_dec_level3, conv3_4], 1)
                inp_dec_level3 = self.reduce_chan_level3(inp_dec_level3)
                conv4_3 = self.conv4_3_RGB(inp_dec_level3) # torch.cat([conv4, F.interpolate(conv4, scale_factor=2, mode=self.upMode)], dim=1)  self.upMode为 bilinear ([18, 512, 12, 12])
                # Second Up  
                inp_dec_level2 = self.up3_2(conv4_3)
                inp_dec_level2 = torch.cat([inp_dec_level2, conv2_3], 1)
                inp_dec_level2 = self.reduce_chan_level2(inp_dec_level2)
                conv3_2 = self.conv3_2_RGB(inp_dec_level2)
                # Third Up
                inp_dec_level1 = self.up2_1(conv3_2)
                inp_dec_level1 = torch.cat([inp_dec_level1, conv1_2], 1)
                conv2_1 = self.conv2_1_RGB(inp_dec_level1)
                # return channel K*K*N
                core3 = self.outc(conv2_1)
...
class VGG16Encoder(nn.Module):
    def __init__(self):
        ...
        # 加载预训练的VGG16模型，并且去掉最后3个全连接层
        base_model = vgg16(pretrained=True).features

        # 构建三个输出
        self.output1 = nn.Sequential(*list(base_model.children())[:4])
        self.output2 = nn.Sequential(*list(base_model.children())[4:9])
        self.output3 = nn.Sequential(*list(base_model.children())[9:16])
